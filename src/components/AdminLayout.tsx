...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const avatar = profile?.photoURL ? (
    ...
    <span className="text-sm font-bold text-teal-700">{profile?.fullName?.[0]?.toUpperCase() || "A"}</span>
  );
        ...
        >
          ...
          <div className="border-t border-slate-100 px-4 py-4">
            <div className="mb-3 flex items-center gap-3 rounded-2xl px-3 py-2.5">
              <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-teal-100">
                {avatar}
              </div>
              <div className="min-w-0">
                <p className="truncate font-semibold text-slate-800">{profile?.fullName || "Clinic Admin"}</p>
                <p className="text-sm text-teal-600">Admin</p>
              </div>
            </div>
            <button
              type="button"
